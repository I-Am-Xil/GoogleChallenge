...
def binarySearch(h, converter):
    ...
    while(True):
        ...
        posible_root_1 = root_2-2**(h-counter)
        posible_root_2 = root_2-1
        
        if ((posible_root_2 == converter) or (posible_root_1 == converter)):
            break
            
        if(posible_root_1 > converter):
            root_2 = posible_root_1
            print(f"converter: {converter}, posible_root_1: {posible_root_1}")
                
        else:
            root_2 = posible_root_2
            print(f"converter: {converter}, posible_root_2: {posible_root_2}")
        
        counter += 1
    
    print(f"converter: {converter}, root_2: {root_2}\n")
    return root_2
...
def solution(h, q):
    p = []
    for converter in q:
        p.append(binarySearch(h, converter))
    return p
